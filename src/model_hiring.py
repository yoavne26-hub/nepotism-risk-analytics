...
EXPLANATORY_FEATURES = EXPLANATORY_MERIT_FEATURES + NEPOTISM_FEATURES + CHANNEL_FEATURES
...
POST_SECONDARY_LEVELS = {"Bachelor", "Master", "Doctorate"}
...
def get_paths() -> dict[str, Path]:
    ...
    output_dir = project_root / "outputs" / "hiring_model"
    return {
        "input": project_root / "data" / "processed" / "candidates_model.csv",
        "output_dir": output_dir,
        "metrics": output_dir / "hiring_metrics.csv",
        "coefficients": output_dir / "hiring_coefficients.csv",
        "predictions": output_dir / "candidate_predictions.csv",
...
def load_data(input_path: Path) -> pd.DataFrame:
    ...
    print(f"Loaded {len(df)} candidate rows from {input_path}")
    return df
...
def ensure_model_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_defaults = {
        "high_school_gpa": np.nan,
        "college_gpa": np.nan,
        "test_score": np.nan,
        "interview_score": np.nan,
    ...
    df["education_level_score"] = (
        df["education_level"].map(education_score_map).fillna(1.0).astype(float)
    )

    for column in numeric_defaults:
        df[column] = pd.to_numeric(df[column], errors="coerce")

    df["hired_flag"] = pd.to_numeric(df["hired_flag"], errors="raise").astype(int)
...
def fit_scenario_specific_explanatory_models(df: pd.DataFrame) -> tuple[list[pd.DataFrame], list[str]]:
    ...
    for scenario_name, scenario_df in df.groupby("scenario"):
        y = scenario_df["hired_flag"].astype(int)
        positives = int(y.sum())
        if len(scenario_df) < 80 or positives < 8 or y.nunique() < 2:
            notes.append(
                f"Skipped scenario-specific explanatory hiring model for {scenario_name} "
                f"(rows={len(scenario_df)}, hires={positives})."
            )
            continue
...
def plot_curves(
    ...
    ax.set_title("Hiring Model ROC Curve")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("Hiring Model Precision-Recall Curve")
    ax.legend()
    fig.tight_layout()
...
def print_probability_summary(observed_hire_rate: float, metrics_df: pd.DataFrame) -> None:
    print(f"\nObserved overall hire rate: {observed_hire_rate:.3%}")
...
def print_model_summary(metrics_df: pd.DataFrame, scenario_notes: list[str]) -> None:
    ...
    print(
        ...
        "because it keeps the merit dimensions separate while exposing the connection terms directly."
    )
    print(
    ...
    print(
        "  - Compact layer: the compact models keep a shorter positive-feature specification while "
        "still separating raw academic and connection inputs."
    )
    if scenario_notes:
...
def main() -> None:
    ...
    observed_hire_rate = float(df["hired_flag"].mean())

    train_df, test_df = train_test_split(
        df,
        test_size=0.30,
        random_state=42,
        stratify=df["hired_flag"],
    )
    train_df = train_df.reset_index(drop=True)
    test_df = test_df.reset_index(drop=True)
    all_df = df.reset_index(drop=True)
    y_train = train_df["hired_flag"]
    y_test = test_df["hired_flag"]

    metrics_rows = []
    coefficient_tables = []
    probability_map: dict[str, tuple[str, np.ndarray]] = {}

    test_index_labels = set(test_df["candidate_id"].astype(str))
    predictions_df = pd.DataFrame(
        {
            "candidate_id": all_df.get("candidate_id"),
            "company_id": all_df.get("company_id"),
            "scenario": all_df.get("scenario"),
            "hired_flag": all_df["hired_flag"],
            "split": np.where(
                all_df["candidate_id"].astype(str).isin(test_index_labels),
                "test",
                "train",
            ),
        }
    ...
    print_probability_summary(observed_hire_rate, metrics_df)
    print("\nSaved outputs:")
